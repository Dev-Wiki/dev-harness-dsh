...
V0 的生产实现前置证据与 K1–K4 编排、状态、Audit、Auto Fix 和授权基线均已完成，S1 可以在稳定 seam 上启动。外部 QA Skill 不作为 V0 的硬依赖：当前环境没有已验证的稳定 QA Skill 输入、授权与完成状态协议，因此 S2 必须按产品选择顺序重新发现能力；在验证出更高优先级 Adapter 前，唯一可声明可用的兜底是生成手工检查清单，且不得把 `MANUAL_REQUIRED` 冒充 `PASS`。
